...
export function BulkImportModal({ isOpen, onClose }: BulkImportModalProps) {
  ...
  const parseTaskLine = (line: string): ParsedTask => {
    ...
    // Extract notes first (everything after #)
    const notesMatch = line.match(/#\s*(.+)$/);
    if (notesMatch) {
      notes = notesMatch[1].trim();
      title = line.replace(notesMatch[0], '').trim();
    }

    // Extract priority markers from the remaining text (case insensitive)
    const priorityMatch = title.match(/-\s*(urgent|high|medium|low)/i);
    if (priorityMatch) {
